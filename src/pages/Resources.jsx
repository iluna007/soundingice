import "../Styles/Resources.css";

const Resources = () => {
	return (
		<div className='container-fluid'>
			<div
				className='mt-6'
				style={{
					textAlign: "justify",
					paddingTop: "48px",
					paddingBottom: "24px",
					paddingLeft: "32px",
					paddingRight: "32px",
				}}
			>
				<div className='row example'>
					<div className='col-2'></div>
					<div className='col'>
						<h2 className='titleResource'>Resources</h2>
						<div className='mt-5'></div>
						<div>
							<h3 className='subtitles'>Readings</h3>
							<ul className='description4'>
								<li>
									<strong>Benoit Turcotte.</strong> “Will There Be an Ice Bridge
									This Winter? Predicting Spatio-Temporal Freeze-up Patterns
									Along the Yukon River, Canada”.
									<a
										href='https://www.iahr.org/library/infor?pid=8556'
										target='_blank'
										rel='noopener noreferrer'
										className='glitch-link'
									>
										Proceedings of the 25th IAHR International Symposium on Ice
										(Trondheim, 2020).
									</a>
								</li>
								<li>
									<strong>
										Burrell, B. C., Beltaos, S., &amp; Turcotte, B.
									</strong>{" "}
									“Effects of climate change on river-ice processes and ice
									jams”. International Journal of River Basin Management, 21(3),
									2022: 421–441.
								</li>
								<li>
									<strong>Julie Cruikshank.</strong> “Are Glaciers ‘Good to
									Think With’? Recognising Indigenous Environmental Knowledge.”
									Anthropological Forum 22(3), 2012: 239–50.
								</li>
								<li>
									<strong>Julie Cruikshank.</strong>{" "}
									<em>
										Do Glaciers Listen? Local Knowledge, Colonial Encounters and
										Social Imagination.
									</em>{" "}
									Vancouver: UBC Press, 2005.
								</li>
								<li>
									<strong>A. Farina, A. Eldridge and P. Li.</strong>{" "}
									“Ecoacoustics and Multispecies Semiosis: Naming, Semantics,
									Semiotic Characteristics, and Competencies” in Biosemiotics,
									14, 2022: 141–165.
								</li>
								<li>
									<strong>Steven Feld.</strong> “Acoustemology: in Keywords in
									Sound”, eds. D. Novak &amp; M. Sakakeeny, Duke University
									Press 2015.
								</li>
								<li>
									<strong>Michael Gates.</strong>{" "}
									<em>
										Hollywood in the Klondike Dawson City’s Great Film Find.
									</em>{" "}
									BC: Harbour Publishing 2022.
								</li>
								<li>
									<strong>Benoit Turcotte.</strong> “Will There Be an Ice Bridge
									This Winter? Predicting Spatio-Temporal Freeze-up Patterns
									Along the Yukon River, Canada”. Proceedings of the 25th IAHR
									International Symposium on Ice (Trondheim, 2020).
								</li>
								<li>
									<strong>Mark Peter Wright.</strong>{" "}
									<em>
										Listening after Nature: Field Recording, Ecology, Critical
										Practice.
									</em>{" "}
									London: Bloomsbury Academic, 2022.
								</li>
								<li>
									<strong>
										<a
											href='https://www.yukonu.ca/sites/default/files/inline-files/Compendium%202021.pdf'
											target='_blank'
											rel='noopener noreferrer'
											className='glitch-link'
										>
											Yukon Climate Change Compendium 2021.
										</a>
									</strong>
								</li>
							</ul>
						</div>
						<div>
							<h3 className='subtitles'>Projects & Podcasts</h3>
							<ul className='description4'>
								<li>
									Alice Eldridge –
									<a
										href='https://www.youtube.com/watch?v=R3KxzxIKsXI'
										target='_blank'
										rel='noopener noreferrer'
										className='glitch-link'
									>
										Learning to Listen.
									</a>
								</li>
								<li>
									<a
										href='https://amkanngieser.com/'
										target='_blank'
										rel='noopener noreferrer'
										className='glitch-link'
									>
										Anja Kanngieser
									</a>
									– Geographer and sound artist exploring listening and
									attunement to the relations between people, place, and
									ecologies.
								</li>
								<li>
									Philip Samartzis –
									<a
										href='https://index.bogongsound.com.au/'
										target='_blank'
										rel='noopener noreferrer'
										className='glitch-link'
									>
										The Bogong High Plains Sound Map.
									</a>
								</li>
								<li>
									Jana Winderen –{" "}
									<a
										href='https://www.youtube.com/watch?v=xCYN6IovKvc'
										target='_blank'
										rel='noopener noreferrer'
										className='glitch-link'
									>
										The Wanderer
									</a>
									, a sound composition from hydrophone recordings of
									zooplankton and phytoplankton in the Atlantic Ocean.
								</li>
								<li>
									<a
										href='https://www.polyphonyproject.com/en'
										target='_blank'
										rel='noopener noreferrer'
										className='glitch-link'
									>
										Polyphony Project.
									</a>
								</li>
								<li>
									Pauline Oliveros –
									<a
										href='https://www.library.ignota.org/book/quantum-listening/'
										target='_blank'
										rel='noopener noreferrer'
										className='glitch-link'
									>
										Quantum Listening.
									</a>
								</li>
								<li>
									<a
										href='https://rfcx.org/'
										target='_blank'
										rel='noopener noreferrer'
										className='glitch-link'
									>
										Rainforest Connection.
									</a>
								</li>
							</ul>
						</div>
						<div>
							<h3 className='subtitles'>Organisations</h3>
							<ul className='description4'>
								<li>
									<a
										href='https://coldcutsvideofestival.wordpress.com/'
										target='_blank'
										rel='noopener noreferrer'
										className='glitch-link'
									>
										Cold Cuts Video Festival
									</a>
								</li>
								<li>
									<a
										href='https://cripe.ca/'
										target='_blank'
										rel='noopener noreferrer'
										className='glitch-link'
									>
										Committee on River Ice Processes and the Environment
									</a>
								</li>
								<li>
									<a
										href='https://www.dawsonfilmfest.com/about.html'
										target='_blank'
										rel='noopener noreferrer'
										className='glitch-link'
									>
										Dawson City Film Fest
									</a>
								</li>
								<li>
									<a
										href='http://dawson.meteomac.com/'
										target='_blank'
										rel='noopener noreferrer'
										className='glitch-link'
									>
										Dawson weather station
									</a>
								</li>
								<li>
									<a
										href='https://kiac.ca/'
										target='_blank'
										rel='noopener noreferrer'
										className='glitch-link'
									>
										Klondike Institute of Art &amp; Culture (KIAC)
									</a>
								</li>
								<li>
									<a
										href='https://www.yukonu.ca/programs/continuing-studies/climate-change'
										target='_blank'
										rel='noopener noreferrer'
										className='glitch-link'
									>
										Yukon University / The Climate Change Research Program
									</a>
								</li>
							</ul>
						</div>
					</div>
				</div>
			</div>
		</div>
	);
};

export default Resources;
